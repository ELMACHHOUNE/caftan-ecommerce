...
export default function ProductDetailPage() {
  ...
  const handleAddToCart = () => {
    if (!product) return
    const sizes = (product.sizes || []).map((s) => s.name || s.value).filter(Boolean)
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-4">
              <div className="relative aspect-[3/4] overflow-hidden rounded-xl bg-muted border-2 border-border">
                ...
                />
                <Badge className="absolute top-4 right-4 bg-accent text-accent-foreground">{product.category?.name || "Product"}</Badge>
              </div>
            ...
            <div className="space-y-6">
              ...
              <Card className="border-2 border-border">
                <CardContent className="p-6 space-y-4">
                  <div className="flex items-center justify-between p-4 border-2 border-border rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-primary">{formatCurrency(product.onSale && product.salePrice ? product.salePrice : product.price, currency)}</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="flex gap-4">
                <Button size="lg" className="flex-1 h-14 text-lg" onClick={handleAddToCart} disabled={sizes.length > 0 && !selectedSize}>
                  ...
                  <ul className="space-y-2">
                    {product.specifications.map((spec, index) => (
                      <li key={index} className="flex items-start gap-2">
                        ...
                        <span className="text-muted-foreground">{spec.name}: {spec.value}</span>
                      </li>
                    ))}
                  </ul>
                </div>
